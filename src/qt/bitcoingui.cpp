    ...
    rpcConsole(0)
{
    ...
    setWindowTitle(tr("Xg"));
    qApp->setStyleSheet("QMainWindow { background:rgb(255,255,255);font-family:'Open Sans,sans-serif'; } #frame { } QToolBar QLabel { padding-top:15px;padding-bottom:10px;margin:0px; } #spacer { background:rgb(0,0,0);border:none; } #toolbar2 { border:none;width:10px; background-color:qlineargradient(x1: 0, y1: 0, x2: 0.5, y2: 0.5,stop: 0 rgb(184,156,48), stop: 1 rgb(184,156,48)); } #toolbar { border:none;height:100%;padding-top:20px; background: rgb(184,156,48); text-align: left; color: white;min-width:200px;max-width:200px;} QToolBar QToolButton:hover {background-color:qlineargradient(x1: 0, y1: 0, x2: 2, y2: 2,stop: 0 rgb(0,0,0), stop: 1 rgb(0,0,0),stop: 2 rgb(39,115,33));} QToolBar QToolButton { font-family:Century Gothic;padding-left:20px;padding-right:100px;padding-top:10px;padding-bottom:10px; width:100%; color: white; text-align: left; background-color: rgb(0,0,0) } #labelMiningIcon { padding-left:5px;font-family:Century Gothic;width:100%;font-size:10px;text-align:center;color:white; } QMenu { background: rgb(0,0,0); color:white; padding-bottom:10px; } QMenu::item { color:white; background-color: transparent; } QMenu::item:selected { background-color:qlineargradient(x1: 0, y1: 0, x2: 0.5, y2: 0.5,stop: 0 rgb(0,0,0), stop: 1 rgb(0,0,0)); } QMenuBar { background: rgb(0,0,0); color:white; } QMenuBar::item { font-size:12px;padding-bottom:12px;padding-top:12px;padding-left:15px;padding-right:15px;color:white; background-color: transparent; } QMenuBar::item:selected { background-color:qlineargradient(x1: 0, y1: 0, x2: 0.5, y2: 0.5,stop: 0 rgb(184,156,48), stop: 1 rgb(184,156,48)); }");
#ifndef Q_OS_MAC
...
statisticsAction = new QAction(QIcon(":/icons/statistics"), tr("&Statistics"), this);
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a xg address"));
...
connect(statisticsAction, SIGNAL(triggered()), this, SLOT(gotoStatisticsPage()));
    ...
    aboutCardAction = new QAction(tr("Xg Website"), this);
    aboutCardAction->setToolTip(tr("Go to Xg website"));
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About Xg"), this);
    aboutAction->setToolTip(tr("Show information about xg"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for xg"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/bitcoin"), tr("&Show / Hide"), this);
    encryptWalletAction = new QAction(QIcon(":/icons/lock_closed"), tr("&Encrypt Xg..."), this);
    encryptWalletAction->setToolTip(tr("Encrypt or decrypt wallet"));
    encryptWalletAction->setCheckable(true);
    backupWalletAction = new QAction(QIcon(":/icons/filesave"), tr("&Backup Xg..."), this);
    backupWalletAction->setToolTip(tr("Backup Xg to another location"));
    changePassphraseAction = new QAction(QIcon(":/icons/key"), tr("&Change Passphrase..."), this);
    changePassphraseAction->setToolTip(tr("Change the passphrase used for wallet encryption"));
    unlockWalletAction = new QAction(QIcon(":/icons/lock_open"), tr("&Unlock Xg..."), this);
    unlockWalletAction->setToolTip(tr("Unlock Xg"));
    lockWalletAction = new QAction(QIcon(":/icons/lock_closed"), tr("&Lock Xg"), this);
    lockWalletAction->setToolTip(tr("Lock Xg"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("xg client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("xg client"));
...
{
    QDesktopServices::openUrl(QUrl("http://www.thexg.in/"));
}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to xg network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid xg address or malformed URI parameters."));
    }
...
void BitcoinGUI::setEncryptionStatus(int status)
{
    ...
    case WalletModel::Unlocked:
        ...
        labelEncryptionIcon->setToolTip(tr("Xg is <b>encrypted</b> and currently <b>unlocked</b>"));
    ...
    case WalletModel::Locked:
        ...
        labelEncryptionIcon->setToolTip(tr("Xg is <b>encrypted</b> and currently <b>locked</b>"));
        encryptWalletAction->setChecked(true);
        changePassphraseAction->setEnabled(true);
...
void BitcoinGUI::backupWallet()
{
    ...
    QString filename = QFileDialog::getSaveFileName(this, tr("Backup Xg"), saveDir, tr("Xg Data (*.dat)"));
    if(!filename.isEmpty()) {
        if(!walletModel->backupWallet(filename)) {
            QMessageBox::warning(this, tr("Backup Failed"), tr("There was an error trying to save the Xg data to the new location."));
        }
    }
}
...
void BitcoinGUI::updateStakingIcon()
{
    ...
    else
    {
        ...
        if (pwalletMain && pwalletMain->IsLocked())
            labelStakingIcon->setToolTip(tr("Not staking because Xg is locked"));
        else if (vNodes.empty())
            labelStakingIcon->setToolTip(tr("Not staking because Xg is offline"));
        else if (IsInitialBlockDownload())
            labelStakingIcon->setToolTip(tr("Not staking because Xg is syncing"));
        else if (!nWeight)
